...
GOAL: 
    ...
    To Do:    
        ...
        Bulk Update ( matric style ) 
            Plot jsut can not keep up with que - as simple as that.  
            Calculatios only take a few milisecons........
            One graph running right now does goo - till i touch the graph - then it goes seconds behins 
                and i don't think catcehs up.  Oh fuck - it jjust get workse and worse.
                
        
        Make graph acceptably usable
            I thought we ha a nice worked out ketp x and y where you put it bt in frame - check the process in stram graph
            ticks size acts a bit wierrd at beggining, we don't nee the numbers,..
            
        Make sure after wards we don't get to far behind the stream and the plot time.
    
    Later:
    
        memcache d
        
        Im tired now.  Was i really right, that simply putting calculated data into sql ( without graphing ) 
            we couldn't close to keep up with quee
...
if __name__ == '__main__':
    ...
    stream = Process(target=price_stream,
                     args=(pairs_index, db, bids_or_asks, q1))
    # Graph indicators and currencies
    graph = Process(target=graph_indicator,
