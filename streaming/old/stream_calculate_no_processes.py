    ...
    for ticks in api.request(r):
        if ticks['type'] == 'PRICE' and ticks['instrument'] in pairs_index:
            try:
                ...
                # Rolling Covariance - might be super slow
                d = {'a': differences[currency_to_graph], 'b': pairs[currency_to_graph]}
                rolling_cov = pd.DataFrame.from_dict(d).rolling(rolling_cov_window).cov().fillna(0).values[:data_points, 1]
...
price_stream(pairs_index, currencies_index, db, bids_or_asks, q, oanda_api, oanda_account)
