...
from libraries.oanda import price_stream
...
Next:
    ...
    Seperate Calculatios process:  
        Also - the que sharing might be a mess
        Without graphing, calculation take only a .01 to complete.
        I on't know why, but with multiple processes, takes much longer.
        So we are going to put the process in the data stream and see what happens
        with a bunch of plots updating
        
        
        
        
        
    OK.  Lets make sure we are doing our calculations in a good time.
    
        OK.  Moving the plot fucks it all up.
    Lets try again - updating in bytes
        Solution 1:yt plot.
            fetch, calculate and update for an rows we might have missed.

    maybe only upate gfraph on right currency ?
    

Notes:  
    y-axis too big
    
    Optional - Only graph each when row is an update to it's own price:
        Times won't match, but helps performane.    
        
    Coul try adding data to line instead of rolling ? - not now
        
    Can we share a plot winow between all graphs - oof.  later
...
# Graphing Queues - don't know a thing about iterators
q1 = Queue()
q2 = Queue()
q3 = Queue()
q4 = Queue()
...
def graph_indicator(pairs_index, currencies_index, q, db, data_points, mean_windows, currency_to_graph):
    ...
    global title
    title = pairs_index[currency_to_graph]
    ...
    # p.setLabel('left', pairs_index[currency_to_graph], units='A',
    ...
    #     global title
    #
    #     curve.setData(y=difference_graph_line)
    #     curve2.setData(y=pair_graph_line)
    #     curve3.setData(y=mean_line_1_graph)
    #     curve4.setData(y=mean_line_2_graph)
    #     curve5.setData(y=mean_line_3_graph)
    #     ptr += 1
    #     now = pyqt_time()
    #     dt = now - lastTime
    #     lastTime = now
...
if __name__ == '__main__':
    ...
    # Streaming Process - imported
    stream = Process(target=price_stream,
                     args=(pairs_index, db, bids_or_asks, q1, q2, q3, q4, q5))
    ...
    g1.start()
    # g2 = Process(target=graph_indicator,
    #                 args=(pairs_index, currencies_index, q2, db, data_points, mean_windows,
    #                       pairs_index.index('EUR_NZD')))
    # g2.start()
    # g3 = Process(target=graph_indicator,
    #                 args=(pairs_index, currencies_index, q3, db, data_points, mean_windows,
    #                       pairs_index.index('AUD_CAD')))
    # g3.start()
    # g4 = Process(target=graph_indicator,
    #                 args=(pairs_index, currencies_index, q4, db, data_points, mean_windows,
    #                       pairs_index.index('GBP_NZD')))
    # g4.start()
    # g5 = Process(target=graph_indicator,
    #                 args=(pairs_index, currencies_index, q5, db, data_points, mean_windows,
    #                       pairs_index.index('AUD_CHF')))
    # g5.start()
